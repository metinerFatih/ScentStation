...
namespace ApplicationCore.Services
{
    public class BasketService : IBasketService
    {
        ...
        {
            var specBasket = new BasketWithItemSpecification(buyerId);
        ...
        {
            var specSourceBasket = new BasketWithItemSpecification(sourceBuyerId);
            var sourceBasket = await _basketRepo.FirstOrDefaultAsync(specSourceBasket);

            if (sourceBasket == null || sourceBasket.BasketItems.Count == 0)
                return;
